...
$(document).ready(function(){
	$('.events').find('.event:first .event_photo').show();
	$('.event_title').click(function(e) {
		e.preventDefault();
		var id = $(this).attr('id').replace('event_', '');
		var day = $(this).attr('id').split('_');
		day = day[day.length-1];
		$('#day_'+day+' .event_photo:not(#event_photo_'+id+')').slideUp();
		$('#event_photo_'+id).slideDown();
		$('#event_photo_'+id).css({display: 'block'});
	});
	...
	if ($("#countdown_timer").length > 0) {
		$('#countdown_timer').countdown({until: $.countdown.UTCDate(-5, 2011, 5 - 1, 5,12),format:"DHM",layout:'<span class="counter_days">{dnn}</span><em>días</em><span class="counter_hours">{hnn}</span><em>horas</em><span class="counter_days">{snn}</span><em>minutos</em>'});
	}
	...
	*/
	$("body.home a.anchorLink").click(function(e){
		...
		color = $(link).attr('href').replace('#', '').replace('index.html/', '');
